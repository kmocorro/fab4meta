...
module.exports = function(io){
    ...
    io.on('connection', function(socket){ // Realtime Andon Board Panel
        ...
        socket.on('dateAndprocess', function(dateAndprocess_obj){   // socket listener for date and process
            //console.log(moment(dateAndprocess_obj[0].dtime).format());
                ...
                dateAndprocess_obj_isValid().then(function(dateAndprocess_obj_valid){
                    function is_shift_AMorPM(){ //  check if AM or PM
                        return new Promise(function(resolve, reject){
                            ...
                            let shift_AM_start = Date.parse(moment(dateAndprocess_obj_valid.dtime).format('YYYYY-MM-DD, 06:40:00')); // adjusting + 20mins to parallel in DB update
                            let shift_AM_end = Date.parse(moment(dateAndprocess_obj_valid.dtime).format('YYYYY-MM-DD, 18:39:59')); // adjusting + 20mins to parallel in DB update
    
                            let shift_PM_start = Date.parse(moment(dateAndprocess_obj_valid.dtime).format('YYYYY-MM-DD, 18:40:00')); // adjusting + 20mins to parallel in DB update
                            ...
                            let shift_PM_end = Date.parse(moment(dateAndprocess_obj_valid.dtime).format('YYYYY-MM-DD, 06:39:59')); // adjusting + 20mins to parallel in DB update
                                ...
                                return fab_hour().then(function(fab_hour_results){
                                    return status_per_tool().then(function(status_results){
                                        ...
                                        for(let i=0;i<oee_value_per_tool_obj.length;i++){ // feed the xy coord LINE
                                            ...
                                            yOEEtarget.push(
                                                (((fab_hour_obj[0].hour/12) * uph_per_tool_obj[i].target_oee) * 100).toFixed(0)  // 12 hours per shift
                                            )
